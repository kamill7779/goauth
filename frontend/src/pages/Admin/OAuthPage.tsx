  ...
  return (
    <div className="animate-[fadeInUp_0.4s_ease]">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-ink mb-1">OAuth Clients</h1>
          ...
          <div className="p-6 space-y-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="flex items-center gap-4 animate-pulse">
                <div className="h-12 w-48 bg-surface-hover rounded" />
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-line">
                  <th className="text-left px-6 py-3.5 text-xs font-semibold text-ink-tertiary uppercase tracking-wider">应用名称</th>
                  <th className="text-left px-6 py-3.5 text-xs font-semibold text-ink-tertiary uppercase tracking-wider">Client ID</th>
                  <th className="text-left px-6 py-3.5 text-xs font-semibold text-ink-tertiary uppercase tracking-wider">Scopes</th>
                  <th className="text-left px-6 py-3.5 text-xs font-semibold text-ink-tertiary uppercase tracking-wider">状态</th>
                  <th className="text-left px-6 py-3.5 text-xs font-semibold text-ink-tertiary uppercase tracking-wider">自动成员</th>
                  <th className="text-left px-6 py-3.5 text-xs font-semibold text-ink-tertiary uppercase tracking-wider">密钥轮换</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-line">
                {clients.map((client) => (
                  <tr key={client.client_id} className="hover:bg-surface-hover transition-colors">
                    <td className="px-6 py-4">
                      <p className="text-sm font-medium text-ink">{client.name}</p>
                      <p className="text-xs text-ink-tertiary mt-0.5">{client.redirect_uris.length} 个回调地址</p>
                    </td>
                    <td className="px-6 py-4">
                      <code className="text-xs font-mono bg-surface-hover px-2 py-1 rounded text-ink-secondary">{client.client_id}</code>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1">
                        {client.scopes.map(s => (
                          <span key={s} className="px-1.5 py-0.5 text-[10px] font-medium bg-surface-hover text-ink-secondary rounded">{s}</span>
                        ))}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <button className="p-1 hover:bg-surface-hover rounded transition-colors" title="轮换密钥">
