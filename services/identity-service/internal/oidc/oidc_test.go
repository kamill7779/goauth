...
func TestCreateClientRecordsAuditLog(t *testing.T) {
	...
	client, err := service.CreateClient(context.Background(), CreateClientInput{
		...
		ClientSecret:            "super-secret",
		Name:                    "Audited Client",
		RedirectURIs:            []string{"https://client.example.com/audit"},
		AllowedScopes:           []string{"openid"},
		GrantTypes:              []string{"authorization_code"},
		TokenEndpointAuthMethod: "client_secret_post",
	})
	...
	if err := service.db.Model(&store.AuditLog{}).
		Where("action = ? AND target_id = ?", audit.ActionOAuthClientChanged, client.ClientID).
		Count(&count).Error; err != nil {
		t.Fatalf("count audit logs: %v", err)
	}
	if count != 1 {
		t.Fatalf("oauth client audit log count = %d, want 1", count)
	}
}
